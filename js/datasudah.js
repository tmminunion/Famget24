...
if (NamaBus == 0) {
  document.getElementById("manifest").textContent =
    "Daftar Peserta yang Belum Absensi";
} else if (NamaBus == 1) {
  document.getElementById("manifest").textContent =
    "Daftar Peserta yang Sudah Absensi";
} else {
...
onValue(starCountRef, (snapshot) => {
  ...
  const urllink = `https://api.bungtemin.net/FamgetAbsensi/resumeALL/${Acara}`;
  fetchAbsensiData(Acara, urllink);
});
...
function fetchAbsensiData(Acara, urllink) {
  ...
  fetch(urllink)
    ...
    .then((data) => {
      ...
      document.getElementById("data_persen").textContent =
        allpresentasi.toFixed(0) + "%";
    })
    .catch((error) => {
      const urllinkne = `https://nextfire-two-ruby.vercel.app/api/getAll/${Acara}`;
      fetchAbsensiData(Acara, urllinkne);
    });
}
